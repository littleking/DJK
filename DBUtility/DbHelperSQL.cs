...
namespace DJK.DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns>DataSet</returns>
        public static DataSet RunProcedureAndGetTotalRecord(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet dataSet = new DataSet();
                ...
                sqlDA.Fill(dataSet);
                //增加序号列【2012-11-14 YM】
                dataSet.Tables[0].Columns.Add("AutoID");
                int intPageIndex = int.Parse(sqlCommand.Parameters["p_curPage"].Value.ToString());
                int intPageSize = int.Parse(sqlCommand.Parameters["p_pageSize"].Value.ToString());
                int intTotalCount = int.Parse(sqlCommand.Parameters["p_totalRecords"].Value.ToString());
                int intCurrentIndex = 0;
                for (int i = (intPageIndex - 1) * intPageSize; i < intPageSize * intPageIndex && i < intTotalCount; i++)
                {
                    dataSet.Tables[0].Rows[intCurrentIndex]["AutoID"] = i + 1;
                    intCurrentIndex++;
                }
                //获取总记录数【2012-01-15 YM】
                dataSet.Tables[0].TableName = sqlDA.SelectCommand.Parameters["p_totalRecords"].Value.ToString();
                connection.Close();
                return dataSet;
            }
        }
